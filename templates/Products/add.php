...
<div id="notification" class="fixed top-0 right-0 m-6 hidden bg-green-500 text-white px-6 py-3 rounded-lg shadow-lg transition-all duration-500 transform translate-x-full">
    ...
    <div class="w-full sm:w-3/4 lg:w-4/5 p-4">
        <div class="bg-white p-6 rounded-md shadow-md">
            ...
            <fieldset>
                <legend class="text-2xl font-semibold text-gray-800"><?= __('Agregar Producto') ?></legend>

                <div class="space-y-4">
                    <!-- Nombre del producto -->
                    <div>
                        <?= $this->Form->control('name', ['label' => 'Nombre', 'class' => 'w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500']) ?>
                    </div>

                    <!-- Descripción del producto -->
                    <div>
                        <?= $this->Form->control('description', ['label' => 'Descripción', 'type' => 'textarea', 'class' => 'w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500']) ?>
                    </div>
            ...
            <div class="flex justify-end mt-6">
                <?= $this->Form->button(__('Agregar'), ['class' => 'bg-gray-700 text-white px-6 py-2 rounded-md hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition',
                'id' => 'submitButton']) ?>
            </div>
            <?= $this->Form->end() ?>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        submitButton.addEventListener('click', function (e) {
            ...
            e.preventDefault();
            showNotification('Por favor, corrija los errores antes de enviar el formulario.', 'error');
        } else {
            // Simular éxito
